...
@Component({
  selector: 'app-membership-plan-edit',
    imports: [
  ...
  templateUrl: './membership-plan-edit.component.html',
  styleUrls: ['./membership-plan-edit.component.scss']
})
export class MembershipPlanEditComponent {
  form: FormGroup;
  planId: number;

  constructor(private router: Router, private route: ActivatedRoute, private membershipPlansService: MembershipPlansService,  private snackBarService: SnackbarService,) {}
  ...
  ngOnInit() {
    ...
    // Read planId from route and load plan details
    const idParam = this.route.snapshot.paramMap.get('planId');
    if (idParam) {
      this.planId = Number(idParam);
      this.membershipPlansService.getMembershipPlansById(this.planId).subscribe({
        next: (plan) => {
          // Expecting fields: planName, planDescription, planCost
          this.form.patchValue({
            name: plan.planName ?? '',
            description: plan.planDescription ?? '',
            price: plan.planCost ?? null
          });
        },
        error: (error) => {
          this.snackBarService.openSnackBar('Error loading plan: ' + error.message, '', []);
        }
      });
    } else {
      this.snackBarService.openSnackBar('No plan id found in route', '', []);
    }
  }
  ...
  onSubmit(event: Event): void {
    ...
    this.membershipPlansService.updateMembershipPlan(this.planId, planData).subscribe({
      next: response => {
